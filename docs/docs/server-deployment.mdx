...
services:
  ...
  archon-server:
    ...
    networks:
      - app-network
    healthcheck:
      test: ["CMD", "python", "-c", "import urllib.request; urllib.request.urlopen('http://localhost:${ARCHON_SERVER_PORT}/health')"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 40s
  ...
  archon-mcp:
    ...
    networks:
      - app-network
    healthcheck:
      test: ["CMD", "python", "-c", "import socket; s=socket.socket(); s.connect(('localhost', int(__import__('os').environ.get('ARCHON_MCP_PORT','8051')))); s.close()"]
      interval: 30s
      timeout: 10s
      retries: 3
  ...
  archon-agents:
    ...
    networks:
      - app-network
    healthcheck:
      test: ["CMD", "python", "-c", "import urllib.request; urllib.request.urlopen('http://localhost:${ARCHON_AGENTS_PORT}/health')"]
      interval: 30s
      timeout: 10s
      retries: 3
...
5. **Access the application**
   - Web UI: http://localhost:${ARCHON_UI_PORT}
   - API Docs: http://localhost:${ARCHON_SERVER_PORT}/docs
   - MCP Connection: http://localhost:${ARCHON_MCP_PORT}/sse
   - Agents API: http://localhost:${ARCHON_AGENTS_PORT}/docs
   - Documentation: http://localhost:${ARCHON_DOCS_PORT}
...
- **Server**: `http://localhost:${ARCHON_SERVER_PORT}/health`
- **MCP**: `http://localhost:${ARCHON_MCP_PORT}/sse` (SSE endpoint)
- **Agents**: `http://localhost:${ARCHON_AGENTS_PORT}/health`
...
lsof -i :${ARCHON_SERVER_PORT}
netstat -tulpn | grep ${ARCHON_SERVER_PORT}

# Use alternative ports in .env
ARCHON_SERVER_PORT=8090
ARCHON_MCP_PORT=8061
```

### Serve Frontend in Production (host-agnostic)

Use the production compose file to build the frontend and serve the static bundle. This keeps browser requests on the same origin and relies on your reverse proxy to route to backend services.

```bash
# Build and run production stack (frontend served via Vite preview)
docker compose -f docker-compose.prod.yml up --build -d
```

For development, use the dev compose file only:
...
Notes:
- No VITE_API_URL is required; the UI uses relative paths.
- Ensure all services share the same user-defined Docker network (default: app-network).
- Add reverse-proxy routes to forward /api and /mcp to the backend containers.
- Socket.IO uses WebSocket upgrade on /socket.io.

### Reverse Proxy Examples (non‑platform specific)
...
server {
  ...
  # API
  location /api/ {
    proxy_pass http://archon-server:${ARCHON_SERVER_PORT};
  }

  # MCP (SSE/HTTP)
  location /mcp/ {
    proxy_pass http://archon-mcp:8051;
    # SSE: avoid buffering and keep long-lived connections
    proxy_buffering off;
...
> Note
> - If your reverse proxy runs inside the same Docker network, target services by name and use container ports (e.g., archon-server:8080, archon-mcp:8051).
> - If your proxy runs on the host outside Docker, use the published host ports (e.g., localhost:${ARCHON_SERVER_PORT}, localhost:${ARCHON_MCP_PORT}).

#### Caddy
```caddyfile
your.domain {
  reverse_proxy / frontend:5173
  reverse_proxy /api* archon-server:${ARCHON_SERVER_PORT}
  reverse_proxy /mcp* archon-mcp:8051
  reverse_proxy /socket.io* archon-server:${ARCHON_SERVER_PORT}
}
```
...
http:
  routers:
    ...
    api:
      ...
      service: api
    mcp:
      rule: Host(`your.domain`) && PathPrefix(`/mcp`)
      service: mcp
  services:
    ui:
      loadBalancer:
    ...
    api:
      loadBalancer:
        servers:
          - url: "http://archon-server:${ARCHON_SERVER_PORT}"
    mcp:
      loadBalancer:
        servers:
...
Best practices:
- Terminate TLS at the proxy and serve the UI over HTTPS. Ensure backend routes are also HTTPS or proxied same-origin to avoid mixed content.
- Keep timeouts generous for SSE/WebSockets (MCP and Socket.IO).
- In production, prefer read-only mounts and resource limits (see examples below).
...
### MCP Readiness Endpoint

- The MCP server (Streamable HTTP) now exposes an HTTP readiness endpoint:
  - GET /health (same host as /mcp)
  - Returns `{ status: 'healthy' | 'initializing', ready: boolean, transport: 'streamable-http' }`
- Use this for reverse proxy/orchestrator health checks. Example docker-compose:

```yaml
healthcheck:
  test: ["CMD", "sh", "-c", "curl -sf http://localhost:${ARCHON_MCP_PORT:-8051}/health | grep -q '\"ready\": true' "]
  interval: 30s
  timeout: 10s
  retries: 3
  start_period: 60s
...
- Set on the API with `ARCHON_MCP_PUBLIC_URL` (e.g., `https://archon-mcp.example.com`). The API returns this as `public_url` via `/api/mcp/config` for the UI to display consistent IDE configs and deeplinks.
- If not set, the API derives it from `X-Forwarded-Proto` and `Host` headers and appends `/mcp`.
