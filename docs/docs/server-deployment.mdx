...
services:
  ...
  frontend:
    ...
    ports:
      - "${ARCHON_UI_PORT}:5173"
    environment:
      - VITE_API_URL=http://localhost:${ARCHON_SERVER_PORT}
    networks:
      - app-network
    healthcheck:
...
Use the production compose overlay to build the frontend and serve the static bundle. This keeps browser requests on the same origin and relies on your reverse proxy to route to backend services.

```bash
# Build and run production stack (frontend served via Vite preview)
docker compose -f docker-compose.yml -f docker-compose.prod.yml up --build -d

# Or use only the production file
